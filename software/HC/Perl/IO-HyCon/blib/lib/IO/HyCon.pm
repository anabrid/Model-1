...
# 30-APR-2020   S. Koeppel  Can now also connect to HyConAVR via TCP/IP instead of Serial device.
...
sub new {
    ...
    our $port;
    my ($poll_interval, $poll_attempts);
    if(exists $config->{serial}) {
        ...
        $poll_interval = $config->{serial}{poll_interval} // 1000;  
        $poll_attempts = $config->{serial}{poll_attempts} //  200;   # and 200 such intervals.
        ...
        $poll_interval = $config->{tcp}{poll_interval};
        $poll_attempts = $config->{tcp}{poll_attempts};
